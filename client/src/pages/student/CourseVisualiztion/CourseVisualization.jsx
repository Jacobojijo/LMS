...
const CourseVisualization = () => {
  ...
  useEffect(() => {
    const fetchCourseProgressAndData = async () => {
      ...
      try {
        ...
        const courseResponse = await axios.get(`${BASE_API_URL}/api/enrollments/user/${user._id}/courses`, {
        ...
        if (courseResponse.data.count > 0) {
          ...
          try {
            const progressResponse = await axios.get(`${BASE_API_URL}/api/progress/course/${enrolledCourse._id}/resume`, {
              headers: { Authorization: `Bearer ${token}` }
            });
  ...
  const updateProgress = async () => {
    ...
    try {
      ...
      await axios.put(`${BASE_API_URL}/api/progress/course/${courseId}`, {
