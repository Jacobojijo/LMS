...
const Courses = () => {
  ...
  return (
    <FadeIn className="w-full bg-teal-50 py-8 sm:py-12 px-2 sm:px-4" delay={0.1}>
      <div className={`max-w-6xl mx-auto grid ${getGridColumns()} gap-3 sm:gap-4 md:gap-6`}>
          ...
          >
            ...
            <div className="p-3 sm:p-4 md:p-6">
              <FadeInUp delay={0.3 + index * 0.05}>
                <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-800 mb-2">{language.title} Classes</h2>
              </FadeInUp>
              ...
              <FadeInUp delay={0.5 + index * 0.05}>
                <div className="text-xs sm:text-sm md:text-base text-gray-700 mb-3 sm:mb-4 prose prose-sm">
                  {getUniqueContent(language).split('\n\n').map((paragraph, i) => (
                    <p key={i} className="mb-1 sm:mb-2 leading-relaxed">{paragraph}</p>
                  ))}
                </div>
              </FadeInUp>
                  ...
                  <div className="flex-shrink-0">
                    ...
                    >
                      ...
                      <svg 
                        ...
                        className={`h-3 w-3 sm:h-4 sm:w-4 ml-1 transition-transform ${expandedCards[language.title] ? 'rotate-90' : ''}`} 
                        viewBox="0 0 20 20" 
                        fill="currentColor"
                      >
      ...
      {/* Pagination Controls */}
      <FadeIn className="flex justify-center mt-6 sm:mt-8 items-center" delay={0.6}>
        <button 
          onClick={goToPreviousPage} 
          disabled={currentPage === 1}
          className={`p-1 sm:p-2 rounded-md mr-1 sm:mr-2 ${currentPage === 1 ? 'text-gray-400 cursor-not-allowed' : 'text-blue-600 hover:bg-blue-100'}`}
        >
        ...
        {displayPageNumbers().map(pageNumber => (
          <button 
            ...
            className={`px-2 py-1 sm:px-3 sm:py-1 mx-0.5 sm:mx-1 text-xs sm:text-base rounded-md ${currentPage === pageNumber ? 'bg-blue-600 text-white' : 'hover:bg-blue-100'}`}
          >
            {pageNumber}
          </button>
        ))}
        ...
        <button 
          ...
          className={`p-1 sm:p-2 rounded-md ml-1 sm:ml-2 ${currentPage === totalPages ? 'text-gray-400 cursor-not-allowed' : 'text-blue-600 hover:bg-blue-100'}`}
        >
      ...
      {/* Page indicator */}
      <FadeIn className="text-center mt-2 text-xs sm:text-sm text-gray-600" delay={0.7}>
