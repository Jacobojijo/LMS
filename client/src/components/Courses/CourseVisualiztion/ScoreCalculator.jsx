  ...
  const calculateScore = () => {
    ...
    questions.forEach((question, index) => {
      ...
      breakdown.push({
        ...
        userAnswer: userAnswers[index] !== undefined ? String.fromCharCode(65 + userAnswers[index]) : 'Not answered',
  ...
  return (
    ...
    >
      ...
      <div className="score-summary mb-6">
        ...
        <div className="mt-4 p-3 rounded-md font-bold text-center" 
          style={{ 
            backgroundColor: passed ? 'rgba(0, 128, 0, 0.2)' : 'rgba(255, 0, 0, 0.2)',
            color: passed ? 'green' : 'red' 
          }}
        >
          {passed ? 'Congratulations! You passed!' : 'You did not meet the passing score. Please review and try again.'}
        </div>
      </div>
      ...
      <div className="navigation-buttons flex justify-between mt-6">
        ...
        >
          {passed ? (isPractice ? 'Continue to Next Topic' : 'Complete Module') : 'Review Content'}
        </button>
      </div>
    </div>
  );
...
export const EnhancedPracticeQuestions = ({ 
  ...
  const allQuestionsAnswered = questions.every((_, idx) => userAnswers[idx] !== undefined);
  ...
  return (
    <div className="question-content p-6 bg-white rounded-lg shadow">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold mb-1">{topic.title} - Practice Questions</h1>
          <p className="text-gray-600">
            {questions.length} question{questions.length !== 1 ? 's' : ''}
          </p>
        </div>
        <div className="text-sm p-2 rounded" style={{backgroundColor: colors.lightTeal}}>
      ...
      {questions.map((question, qIdx) => (
        ...
        >
          <p className="text-lg font-medium mb-4">Question {qIdx + 1}: {question.question}</p>
              ...
              <div 
                ...
                style={{
                  backgroundColor: !isSubmitted 
                    ? (userAnswers[qIdx] === optIdx ? colors.lightRose : colors.white)
                    : (showAnswers && optIdx === question.correctAnswer 
                        ? '#d1ffd1' 
                        : (userAnswers[qIdx] === optIdx ? '#ffe6e6' : colors.white)),
                  border: "1px solid " + (userAnswers[qIdx] === optIdx ? colors.accent : '#ddd'),
                }}
                onClick={() => !isSubmitted && setUserAnswers({...userAnswers, [qIdx]: optIdx})}
              >
                <div 
                  className="option-circle w-8 h-8 rounded-full flex items-center justify-center mr-3" 
                  style={{
                    backgroundColor: userAnswers[qIdx] === optIdx ? colors.accent : '#f0f0f0',
                    color: userAnswers[qIdx] === optIdx ? colors.white : colors.darkText
                  }}
                >
          ...
          {isSubmitted && showAnswers && (
            <div className="explanation mt-6 p-4 rounded-md" style={{backgroundColor: colors.lightRose}}>
...
export const EnhancedAssessment = ({ 
  ...
  if (!questions || questions.length === 0) return <div>No assessment questions available</div>;
    ...
    <div className="question-content p-6 bg-white rounded-lg shadow">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center">
          <div className="text-sm p-2 rounded mr-4" style={{backgroundColor: colors.lightTeal}}>
            <div>Passing Score: {module.cat.passingScore}%</div>
          </div>
          <div 
            className="text-sm p-2 rounded font-bold" 
            style={{
              backgroundColor: timeRemaining < 60 ? '#ffe6e6' : colors.lightBeige
            }}
          >
